function [basis,ForwardOp,peqoversample,peq,qest,normalizer,X,epsilon] = BuildModelVBDMw(x,k,k2,nvars,delays,shifts,dim)
...
ForwardOp = (basis(1+shifts:T,:).*sqrt(w'))'*(basis(1:T-shifts,:).*sqrt(w').*repmat(peq(1:T-shifts)./qest(1:T-shifts),1,nvars));
